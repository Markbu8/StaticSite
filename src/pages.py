...
def generate_page(from_path, template_path,base_path, dest_path):
    ...
    new_html = new_html.replace('href="/',f'href="{base_path}').replace('src="/',f'src="{base_path}')
...
def generate_pages_recursive(dir_path_content, template_path, base_path, dest_dir_path):
    ...
    for f_or_f in file_folder_list:
        ...
        if os.path.isfile(src_path):
            if src_path[-3:] == ".md":
                generate_page(src_path,template_path,base_path,dest_path.replace(".md",".html"))
        elif os.path.isdir(src_path):
            generate_pages_recursive(src_path,template_path,base_path,dest_path)